...
def make_prediction(pipe):
    ...
    # It reshapes the NumPy array into a 2D array with a specific shape.
    #The 1 in (1,-1) means that the resulting array will have one row. This single row represents the single data point you want to predict.
    #The -1 is a placeholder that tells NumPy to automatically figure out the number of columns based on the number of elements in your array. Since you have 6 variables (miles, year, make, etc.), the final shape will be (1, 6).
    X_pred = np.array([miles, year, make, model, engine_size, province]).reshape(1,-1)#the line takes your six input values and packages them into a (1, 6) NumPy array, which is the required format for making a single prediction with a scikit-learn model.
...
if __name__ == "__main__":
    st.title("🍁2nd Hand Car price predictor")
    ...
    with st.form(key="form"):
        ...
        with col1:
            st.number_input("Miles", value=86132.0, min_value=0.0, step=0.1, key="miles") #step means when user cleck in UI to increment/decrement
       ...
       'Prius c', 'Prius C', 'Prius v']) # options parameter used with index
        with col2:
